...
def parser_args():
    available_models = ['Base-R101-448','Base-TResNetL-448']
    parser = argparse.ArgumentParser(description='PyTorch Training for Multi-label Image Classification')
    parser.add_argument('--dataname', help='dataname', default='coco14', choices=['coco14', 'voc2007', 'voc2012', 'nus_wide', 'vg500'])
    ...
    parser.add_argument('--frozen_layers', action='store_true', default=False,
                        help='apply frozen head of the backbone in train')
    
    parser.add_argument('--optim', default='AdamW', type=str, choices=['AdamW', 'Adam_twd'],
                        help='which optim to use')

    parser.add_argument('-a', '--arch', metavar='ARCH', default='Base-TResNetL-448',
                        choices=available_models,
                        help='model architecture: ' +' | '.join(available_models) +
                            ' (default: Base-TResNetL-448)')
